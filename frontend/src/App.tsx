...
function GlowBtn({ children, onClick, color = C.cyan, outline = false, style = {} }: { children: React.ReactNode; onClick?: () => void; color?: string; outline?: boolean; style?: React.CSSProperties }) {
  ...
  return (
    <button
      ...
      style={{
        background: outline ? 'transparent' : hover ? `${color}22` : `${color}11`,
...
function Card({ children, style = {}, glowColor = C.cyan }: { children: React.ReactNode; style?: React.CSSProperties; glowColor?: string }) {
  return (
    <div style={{
      background: C.panel,
...
function Metric({ label, value, color = C.cyan, sub }: { label: string; value: React.ReactNode; color?: string; sub?: string }) {
  return (
...
function SeverityBadge({ level }: { level: string }) {
  const map: Record<string, string> = { critical: C.red, high: C.amber, medium: C.magenta, low: C.cyan };
...
function Sidebar({ active, setPage }: { active: string; setPage: (page: string) => void }) {
  return (
    <div style={{
      width: 64,
      background: C.bg2,
    ...
    <div style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
      ...
      <div style={{ padding: '60px 40px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
          {FEATURES.map(f => (
            <Card key={f.title} glowColor={f.color} style={{ cursor: 'pointer', transition: 'transform 0.2s' }} onClick={() => setPage(f.page)}>
...
function FileGridPage() {
  ...
  function parseTree(text: string) {
    ...
    const root: any[] = [];
    const stack: any[] = [{ children: root, indent: -1 }];
    lines.forEach(line => {
  ...
  function TreeNode({ node, depth = 0 }: { node: any; depth?: number }) {
    ...
    return (
      <div>
        ...
        {node.isFolder && open && node.children.map((c: any, i: number) => <TreeNode key={i} node={c} depth={depth + 1} />)}
      </div>
    );
  }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          <Card glowColor={C.cyan}>
            ...
            <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
              <GlowBtn onClick={() => setTree(parseTree(input))} color={C.cyan}>Generate Tree</GlowBtn>
          ...
          <Card glowColor={C.emerald} style={{ gridColumn: '1 / -1' }}>
            ...
            <div style={{
              height: 100,
              border: `2px dashed ${C.emerald}44`,
              borderRadius: 6,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
        ...
        {[
          ...
          <button key={item.id} onClick={() => setSideTab(item.id)}
            style={{
              ...
              border: 'none',
              borderLeft: `2px solid ${sideTab === item.id ? C.magenta : 'transparent'}`,
      ...
      <div style={{ display: 'flex', gap: 24, justifyContent: 'center', flexWrap: 'wrap', maxWidth: 900, margin: '0 auto' }}>
        {PLANS.map(p => (
          <div key={p.name} style={{
            ...
            <GlowBtn color={p.color} onClick={() => setPage('login')} style={{ width: '100%', boxSizing: 'border-box' }}>
              Get Started
            </GlowBtn>
          </div>
        ))}
...
function AuthPage({ setPage }: { setPage: (page: string) => void }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {mode === 'signup' && <input type='password' placeholder='Confirm password' style={inputStyle} />}

        <GlowBtn color={C.cyan} onClick={() => setPage('client')} style={{ width: '100%', boxSizing: 'border-box', padding: '12px', marginTop: 8 }}>
...
const PAGE_META: Record<string, { title: string; module: string }> = {
...
function Topbar({ page }: { page: string }) {
  const meta = PAGE_META[page] || { title: '', module: '' };
  return (
    <div style={{
      height: 48,
      background: C.bg2,
...
export default function App() {
  ...
  const PAGES: Record<string, React.ComponentType<any>> = { landing: LandingPage, filegrid: FileGridPage, sentinel: SentinelPage, client: ClientPage, admin: AdminPage, pricing: PricingPage, login: AuthPage };
